...
async def test_redis():
    await add_key_value_redis("greeting", "hello from redis", expire=120)
    value = await get_value_redis("greeting")
    return {"value": value}
...
async def authorize_hubspot_integration(playload : IntegrationAuthRequest):
    return await authorize_hubspot(playload.user_id, playload.org_id)
...
async def get_hubspot_credentials_integration(playload : HubspotCredentialRequest):
    return await get_hubspot_credentials(playload.user_id, playload.org_id)


@app.post('/integrations/hubspot/get_hubspot_items')
async def load_hubspot_data_integration(payload: CredentialsRequest):
    return await get_items_hubspot(payload.dict())
